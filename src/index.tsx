import { Action, ActionPanel, Form, List, showToast, Toast, getPreferenceValues, LaunchProps, Icon } from "@raycast/api";
...
export default function FindYourRepo(props: LaunchProps<{ arguments: Arguments }>) {
    ...
    return (
        <List isLoading={isLoading} searchBarPlaceholder="搜索仓库..."
            children={
                (repos.length === 0 && !isLoading) ? (
                    ...
                    <List.Item
                        ...
                        subtitle={repo.path}
                        accessories={[
                            { text: `${repo.remotes.length} remote(s)` }
                        ]}
                        actions={
                            <ActionPanel>
                                <Action.ShowInFinder
